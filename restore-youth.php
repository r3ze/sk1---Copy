...
if (isset($_SESSION['email'])) {
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM archived_youth_records WHERE id = $id";
        ...
        if ($result->num_rows > 0) {
            ...
            $archiveSql = "INSERT INTO youth_barangay (first_name, middle_name, last_name, gender, contact_no, birthdate, marital_status, religion, osy, ws, yp, pwd, reg_date, session_id) VALUES ('" . $row['first_name'] . "', '" . $row['middle_name'] . "', '" . $row['last_name'] . "', '" . $row['gender'] . "', '" . $row['contact_no'] . "', '" . $row['birthdate'] . "', '" . $row['marital_status'] . "' , '" . $row['religion'] . "' , '" . $row['osy'] . "' ,'" . $row['ws'] . "' , '" . $row['yp'] . "', '" . $row['pwd'] . "' , '" . $row['reg_date'] . "' , '" . $row['session_id'] . "')";

            if ($conn->query($archiveSql) === TRUE) {
                // After archiving, delete the record from the original table
                $deleteSql = "DELETE FROM archived_youth_records WHERE id = $id";
