...
<body>
    ...
    if (isset($_POST['register'])) {
        ...
        $check_sql = "SELECT * FROM municipality WHERE username = '$admin_id'";
        $result = mysqli_query($conn, $check_sql);
        if (mysqli_num_rows($result) > 0) {
            echo "<p class='error'>Municipal Username already exists!</p>";
        } else {
            // Insert the new admin
            $sql = "INSERT INTO municipality (username, password, municipal, role) VALUES ('$admin_id', '$password_hash', '$municipal', 'municipal')";
