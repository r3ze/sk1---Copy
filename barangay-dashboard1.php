...
if (isset($_SESSION['session_id'])) {
  ...
  $sql = "SELECT street, municipal, province, first_name, middle_name, last_name FROM barangay WHERE session_id = '$session_id'";
...
FROM youth_barangay WHERE session_id='$session_id' ";
  ...
  $sql_status = "SELECT status FROM barangay WHERE session_id='$session_id'";
  ...
  if ($result_status) {
    ...
    // $popupDisplay1 = ($status === "Decline") ? "block" : "none";
  } else {
  ...
  $sql2 = "SELECT * FROM barangay_official WHERE session_id = '$session_id'";
  ...
  header("Location: barangay_sample.php");
  exit();
}
?>
...
<?php if($row_status["status"]=== 'Pending'): ?>
...
<?php elseif($row_status["status"] === 'Decline'): ?>
  <style>
    .popup{
      display:block;
    }
