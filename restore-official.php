...
if (isset($_SESSION['email'])) {
    ...
    if (isset($_GET['id'])) {
        ...
        $sql = "SELECT * FROM archived_official_records WHERE id = $id";
        ...
        if ($result->num_rows > 0) {
            ...
            $archiveSql = "INSERT INTO barangay_official (barangay_name, first_name, middle_name, last_name, gender, contact_no, committee, barangay_position, session_id) VALUES ('" . $row['barangay_name'] . "', '" . $row['first_name'] . "', '" . $row['middle_name'] . "', '" . $row['last_name'] . "', '" . $row['gender'] . "', '" . $row['contact_no'] . "', '" . $row['committee'] . "', '" . $row['barangay_position'] . "' ,  '" . $row['session_id'] . "')";

            if ($conn->query($archiveSql) === TRUE) {
                // After archiving, delete the record from the original table
                $deleteSql = "DELETE FROM archived_official_records WHERE id = $id";
